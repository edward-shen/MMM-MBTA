Module.register("MMM-MBTA", {
    ...
    fetchRoute: function(data, updateDomAfter, pred, routeId, tripId, alertId, rawData) {
        ...
        routeRequest.onreadystatechange = function() {
            if (this.readyState === 4) {
                if (this.status === 200) {
                    self.fetchTrip(data, updateDomAfter, pred, JSON.parse(this.response), tripId, alertId, rawData, deferredPromise)
                } else if (this.status === 404) {
                    self.fetchTrip(data, updateDomAfter, pred, "Unavailable", tripId, alertId, rawData, promise);
                }
            }
        };
    ...
    fetchTrip: function(data, updateDomAfter, pred, routeParse, tripId, alertId, rawData, promise) {
        ...
        tripRequest.onreadystatechange = function() {
            if (this.readyState === 4) {
                if (this.status === 200) {
                    self.fetchAlerts(data, updateDomAfter, pred, routeParse, JSON.parse(this.response), alertId, rawData, promise);
                } else if (this.status === 404) {
                    self.fetchAlerts(data, updateDomAfter, pred, routeParse, "Unavailable", alertId, rawData, promise);
                }
            }
        };
    ...
    fetchAlerts: function(data, updateDomAfter, pred, routeParse, tripParse, alertId, rawData, promise) {
        if (alertId === "No alerts") {
            ...
            alertRequest.onreadystatechange = function() {
                if (this.readyState === 4) {
                    if (this.status === 200) {
                        self.processData(data, updateDomAfter, pred, routeParse, tripParse, JSON.parse(this.response), rawData);
                        promise.resolve();
                    } else if (this.status === 404) {
                        self.processData(data, updateDomAfter, pred, routeParse, tripParse, "Unavailable", rawData);
                        promise.resolve();
                    }
                }
            };
    ...
    loopData: function(data, updateDomAfter) {
        ...
        if (data.data.length === 0) {
            this.stationData.push({
                routeType: "9",
                routeId: "None",
                tripSign: "No vehicles are scheduled",
                directionId: "",
                alert: "No alerts",
                preDt: "None",
                preArr: "None",
            ...
            for (let pred = 0; pred < data["data"].length; pred++) {
                ...
                if (data.data[pred].relationships.alerts.data.length === 0) {
                    alertId = "No alerts"
                } else {
                    alertId = data.data[pred].relationships.alerts.data[0].id;
                }

                promises.push(this.fetchRoute(data, updateDomAfter, pred, routeId, tripId, alertId, rawData));
            }
    ...
    processData: function(data, updateDomAfter, pred, routeParse, tripParse, alertParse, rawData) {
        ...
        if (tripParse === "Unavailable") {
            tripSign = "Destination unavailable";
            directionId = "";
        } else {
            tripSign = tripParse.data.attributes.headsign;
            directionId = tripParse.data.attributes.direction_id.toString();
        }
        if (alertParse === "No alerts") {
            alert = "No alerts"
        } else {
            alert = alertParse.data.attributes.header;
        }
    ...
    cleanData: function(data, rawData, updateDomAfter) {
        ...
        // Puts one entry back in if there are no trips within maxTime
        if (this.stationData.length === 0) {
            this.stationData.push({
                routeType: "9",
                routeId: "None",
                tripSign: "No vehicles in your timeframe",
                alert: "No alerts",
                preDt: "None",
                preArr: "None",
                preETA: "None"
